...
def _Fusion(inputs):
    ...
    for input in inputs:
        with tf.name_scope('fusion_%s' % input.name[:-2]):
            w = tf.Variable(tf.random_uniform(shape=[10, 10, 2], dtype=tf.float32), name='w')
            output = input * w
...
def gen_batch(x, y, index, batch_size=32):
    xc, xp, xt = x[0], x[1], x[2]
    begin = index * batch_size
    end = begin + batch_size
    x_batch = [xc[begin:end], xp[begin:end], xt[begin:end], ]
    y_batch = y[begin:end]
    if len(x_batch[0]) < batch_size:
        for i in x_batch:
            i.extend([i[-1]] * (batch_size - len(x_batch[0])))
        y_batch.extend([y_batch[-1]] * (batch_size - len(x_batch[0])))
...
class STResNet():
    ...
    def evaluate(self, mmn, x, y):
        ...
        with tf.Session() as sess:
            ...
            for batch in range(n_chunk):
                ...
                if len(x_batch[0]) != 32: continue
                feed = {self.input_xc: x_batch[0],
                        self.input_xp: x_batch[1],
                        self.input_xt: x_batch[2],
                        self.input_ext: np.zeros(shape=(self.batch_size, self.external_dim)),
    ...
    def predict(self, mmn, x, y):
        ...
        with tf.Session(config=config) as sess:
            ...
            for batch in range(n_chunk):
                x_batch, y_batch = gen_batch(x, y, batch, batch_size=self.batch_size)
                pre = sess.run(predict, feed_dict={self.input_xc: x_batch[0],
                        self.input_xp: x_batch[1],
                        self.input_xt: x_batch[2],
                        self.input_ext: np.zeros(shape=(self.batch_size, self.external_dim))})
                result.extend(list(pre))  
