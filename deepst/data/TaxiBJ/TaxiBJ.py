...
def load_dataset(T=48, nb_flow=2, len_closeness=None, len_period=None, len_trend=None,
    ...
    for year in range(5, 6):
        fname = os.path.join(
            DATAPATH, 'Nyc{}_M10x10_T20_InOut.h5'.format(year))
...
def load_data(len_closeness, len_period, len_trend, len_test, meta_data=True, meteorol_data=True, holiday_data=True, T=72):
    ...
    if os.path.exists(fname):
        ...
        X_train, Y_train, X_test, Y_test, mmn, external_dim, timestamp_train, timestamp_test = \
            load_dataset(T=T, len_closeness=len_closeness, len_period=len_period, len_trend=len_trend,
                         len_test=len_test)
